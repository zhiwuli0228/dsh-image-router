...
export function installSettings(env, schema, base, onChange) {
	...
	env.ctx.inject(['settings'], (sctx) => {
		try {
			...
			// The installed section is audited in SHAPE only, never its values: which
			// fields arrived decides whether the host layer can resolve a route from it,
			// and "the section is registered yet the composed route stays in force" is
			// otherwise invisible.
			const installed = scope.get()
			const asObject = installed !== null && typeof installed === 'object' ? installed : undefined
			const vision = asObject?.vision
			const visionKeys = vision !== null && typeof vision === 'object' ? Object.keys(vision).join(',') : typeof vision
			const endpointKeys = vision !== null && typeof vision === 'object' && vision.endpoint !== null && typeof vision.endpoint === 'object' ? Object.keys(vision.endpoint).join(',') : 'none'
			env.trace(`settings-registered ns=${SETTINGS_NAMESPACE} keys=${asObject === undefined ? typeof installed : Object.keys(asObject).join(',')} visionKeys=${visionKeys} endpointKeys=${endpointKeys}`)
			notify()
		} catch (error) {
