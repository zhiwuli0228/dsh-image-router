...
export function createEnv(ctx, raw, resolved, trace) {
	...
	return {
		...
		get config() {
			...
			// Shape-only audit of the resolution inputs. A section that registered but
			// kept resolving to the composed route is otherwise indistinguishable from
			// one that never arrived, and the merged object is the only place the two
			// shapes meet.
			if (!configTraced) {
				configTraced = true
 ...
 *
 * Two properties here are load-bearing, and both were found by auditing a live
 * configuration that looked ignored:
 *
 *  - **The Endpoint tier's `apiKey` is dropped.** The settings section is handed
 *    back by `describe()` as the resolved value — which includes the user layer —
 *    and a configuration surface seeds its editor from that. Leaving the key in
 *    meant a save (or even an unrelated edit) round-tripped it into
 *    `$DSH_HOME/settings.yaml` as plain text.
 *  - **When an Endpoint is configured, `vision.provider`/`vision.model` are
 *    dropped too.** The section base is part of the value the host composes and
 *    hands back, so including the *resolved* route made it look explicitly chosen.
 *    An explicitly chosen route outranks an Endpoint, so the plugin then routed to
 *    the composed route and never touched the endpoint the user configured —
 *    observed live as `visionKeys=provider,model,endpoint -> qwen-token-plan-cn/…`
 *    while the section's own file held only `endpoint`. The resolved values still
 *    reach the card; they are simply derived from the Endpoint instead of being
 *    written into the section beside it.
 *
 * @param resolved - a normalized configuration.
 * @returns the schema-shaped subset, with no credential in it.
 */
export function configBase(resolved) {
	...
	// `resolved.vision` IS the endpoint route when an Endpoint is configured, so
	// carrying it into the base would state the endpoint's own provider/model as an
	// independent explicit choice.
	const vision = endpoint === undefined
		? {
			...(resolved.vision.provider === undefined ? {} : { provider: resolved.vision.provider }),
			...(resolved.vision.model === undefined ? {} : { model: resolved.vision.model }),
