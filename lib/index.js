...
function readRoute(value, label, problems) {
	...
	// A schema may materialize an unset optional object as `{}`, or an unset optional
	// string as `''`. Both are "absent", not "configured with missing fields": an
	// empty string that counted as present would make the route look explicitly
	// chosen, which silently outranks the Endpoint tier.
	const hasProvider = typeof provider === 'string' && provider.length > 0
	const hasModel = typeof model === 'string' && model.length > 0
	if (!hasProvider && !hasModel && reasoningEffort === undefined) return undefined
	if (!hasProvider) problems.push(`${label}.provider must be a non-empty string`)
	if (!hasModel) problems.push(`${label}.model must be a non-empty string`)
...
export function normalizeConfig(raw) {
	...
	const configuredEndpoint = readEndpoint(visionInput.endpoint, 'vision.endpoint', problems)
	// The key is read from the configuration, then dropped from the resolved result.
	// It is a credential, and this value is what the plugin's every later decision
	// reads (`env.config`) and what its settings section is composed from — so a key
	// left here is a key that can be logged, rendered or persisted. The endpoint tier
	// takes the plaintext from the raw configuration at the one moment it needs it.
	const endpoint = configuredEndpoint === undefined
		? undefined
		: (() => {
			const { apiKey, ...rest } = configuredEndpoint
			void apiKey
			return rest
		})()
	const endpointRoute = endpoint === undefined ? undefined : routeForEndpoint(endpoint)
	/**
	 * Which tier supplies the vision route.
	 *
	 * The Endpoint wins whenever it is genuinely configured, because it is the more
	 * complete specification: it carries the address and the credential, and its model
	 * *is* the route. An explicit `provider`/`model` sitting beside it is normally the
	 * residue of the deployment's own default route — the entry's configuration — and
	 * letting that residue win is exactly how a saved endpoint was ignored in the live
	 * deployment. A deliberate by-hand route+endpoint pair still works: the compose
	 * step (`mergeOverrides`) keeps the user's own route and drops its endpoint, so
	 * this rule never has to arbitrate between two deliberate choices.
	 */
	const explicitRoute = readRoute(config.vision, 'vision', problems)
	const vision = endpointRoute ?? explicitRoute
	if (vision === undefined) {
...
async function analyzeImages(env, refs, sessionId, signal) {
	...
	/**
	 * One attempt, reported as data rather than as a decision.
	 *
	 * The `finish` chunk carries `reason` as an OBJECT — `mapStopReason` answers
	 * `{ kind: 'stop' | 'max-tokens' | 'tool-calls' | 'error' | … }`, with the failure
	 * under `reason.failure`. Comparing it to a string therefore missed every failure
	 * the adapter reported, including its own "completed response with no content",
	 * and turned a diagnosable error into a silent empty answer.
	 */
	const attempt = async (maxTokens) => {
		...
		const shapes = []
		try {
			for await (const chunk of llm.stream({ ...options, maxTokens })) {
				if (chunk === null || typeof chunk !== 'object') continue
				const type = typeof chunk.type === 'string' ? chunk.type : '(no type)'
				if (shapes.length < 12 && !shapes.includes(type)) shapes.push(type)
				if (chunk.type === 'text-delta' && typeof chunk.text === 'string') {
					text.value += chunk.text
				} else if (chunk.type === 'reasoning-delta') {
					sawReasoning = true
				} else if (chunk.type === 'block-end' && chunk.block?.type === 'text') {
					// The adapter emits the completed block as well as any deltas
					// (`text_end` → `block-end` with `block: { type: 'text', text }`), and a
					// provider that does not stream deltas delivers the whole answer ONLY
					// this way: reading just `text-delta` produced an empty analysis with a
					// `stop` finish and no error anywhere.
					if (typeof chunk.block.text === 'string' && chunk.block.text.length > text.value.length) text.value = chunk.block.text
				} else if (chunk.type === 'finish') {
					const reason = chunk.reason ?? chunk.finishReason
					if (reason !== null && typeof reason === 'object') {
						finishReason = typeof reason.kind === 'string' ? reason.kind : 'unknown'
						failureCode = typeof reason.failure?.code === 'string' ? reason.failure.code : undefined
						if (reason.kind === 'error') failure = reason.failure?.message ?? failureCode ?? 'error'
						else if (reason.kind === 'aborted') failure = 'aborted'
					} else if (typeof reason === 'string') {
						finishReason = reason
						if (reason === 'error' || reason === 'aborted') failure = reason
					}
				} else if (chunk.error !== undefined && failure === undefined) {
					failure = chunk.error?.message ?? String(chunk.error)
				}
			}
		} catch (error) {
		...
		return {
			...
			failureCode,
			sawReasoning,
			observed: `chunks=${shapes.length === 0 ? 'none' : shapes.join('+')}${finishReason === undefined ? '' : ` finish=${finishReason}`}`
		}
	}

	const first = await attempt(env.config.maxTokens)
	const firstText = first.text.trim()
	// A failure is never answered with partial text. A truncated analysis is worse than
	// no analysis: nothing downstream knows it stops mid-sentence, whereas leaving the
	// image in place lets the prompt's own path judge it and report the real reason.
	if (first.failure === undefined && firstText.length > 0) return firstText

	/**
	 * On this protocol the token budget is shared with the model's own reasoning
	 * ("reasoning and the answer share `max_tokens` here, so an uncapped reasoning
	 * phase can consume the whole response and leave no answer" — pi-ai's own note).
	 * A budget that suits a plain model can therefore be spent entirely on thinking and
	 * the answer never starts. One retry with room for both beats failing on a setting
	 * the user has no way to reason about.
	 */
	const retryBudget = Math.min(Math.max(env.config.maxTokens * 4, 4096), 32768)
	const budgetShared = first.failureCode === 'EMPTY_RESPONSE' || (first.failure === undefined && first.sawReasoning)
	if (retryBudget > env.config.maxTokens && budgetShared) {
		env.trace(`digest-retry reason=reasoning-consumed-budget budget=${env.config.maxTokens}->${retryBudget} images=${refs.length} ${first.observed}`)
		const second = await attempt(retryBudget)
		const retried = second.text.trim()
		if (second.failure === undefined && retried.length > 0) {
			env.trace(`digest-retry-ok budget=${retryBudget} chars=${retried.length}`)
			return retried
		}
		env.trace(`digest-empty images=${refs.length} budget=${retryBudget} ${second.observed} — no text even with room for the reasoning${second.failure === undefined ? '' : ` (${second.failure})`}`)
		return undefined
	}

	if (first.failure !== undefined) {
		env.trace(`digest-failed reason=${first.failure}${first.failureCode === undefined ? '' : ` code=${first.failureCode}`} images=${refs.length} ${first.observed}`)
		return undefined
	}
	// An empty answer is the one failure a user cannot diagnose from the outside: the
	// route resolved, the credential resolved, and nothing came back. The chunk shapes
	// and the finish reason say whether the adapter produced a different shape than
	// expected, stopped on a length budget, or answered nothing at all.
	env.trace(`digest-empty images=${refs.length} budget=${env.config.maxTokens} ${first.observed} — the route produced no text`)
	return undefined
}
...
async function rewritePrompt(env, request, signal) {
	...
	// The route is recorded on the decision line, not just the mount line: which
	// route actually served an image is the question a user asks when a card's saved
	// configuration looks ignored, and a mount-time value cannot answer it.
	env.trace(`digest session=${typeof sessionId === 'string' ? sessionId : '?'} vision=${env.config.vision.provider}/${env.config.vision.model} images=${images.length} chars=${analysis.length}${names.length === 0 ? '' : ` names=${names.join(',')}`}`)
...
function mergeOverrides(raw, overrides) {
	...
	// `vision` merges field by field rather than wholesale, and an empty value never
	// overwrites one that carries information. The section is composed from a
	// schema-shaped base, so it can name only some of the vision fields the composed
	// configuration already has — or none at all.
	//
	// The precedence between the two tiers follows which one the user actually spoke
	// through, because the section is the user's own layer and `raw` is the entry's
	// configured default:
	//
	//  - the section names an Endpoint → that Endpoint is the configuration; a route
	//    beside it is the entry's default and must not survive (this is exactly how a
	//    saved endpoint was ignored live: `visionKeys=provider,model,endpoint ->
	//    qwen-token-plan-cn/…` while the section's own file held only `endpoint`);
	//  - the section names a route → that route is the configuration, even if the
	//    default carried an Endpoint;
	//  - the section names neither → the default decides, Endpoint included.
	const composed = { ...raw, ...layer }
	const baseVision = raw !== null && typeof raw === 'object' && raw.vision !== null && typeof raw.vision === 'object' && !Array.isArray(raw.vision) ? raw.vision : undefined
	const layerVision = layer.vision
	if (baseVision !== undefined && layerVision !== null && typeof layerVision === 'object' && !Array.isArray(layerVision)) {
		const layerRoute = (typeof layerVision.provider === 'string' && layerVision.provider.length > 0) || (typeof layerVision.model === 'string' && layerVision.model.length > 0)
		// Start from the default only when the section does not decide the tier itself.
		const mergedVision = layerVision.endpoint !== undefined || layerRoute ? {} : { ...baseVision }
		if (layerRoute && mergedVision.endpoint === undefined && baseVision.endpoint !== undefined) {
			// The user chose a route, so the default's endpoint must not also be present.
			delete mergedVision.endpoint
		}
		for (const [key, value] of Object.entries(layerVision)) {
			if (value === undefined || value === null) continue
			if (typeof value === 'string' && value.length === 0) continue
			if (typeof value === 'object' && !Array.isArray(value) && Object.keys(value).length === 0) continue
			mergedVision[key] = value
		}
		composed.vision = mergedVision
	}
	return composed
}
 ...
 */
export function createEnv(ctx, raw, resolved, trace) {
	let overrides
	/**
	 * The last override layer that validated, consulted only while the *current*
	 * layer is invalid. It is deliberately not a cache of "the configuration in
	 * force": with such a cache, clearing the section (which the settings disposer
	 * does) kept serving the override it had last accepted instead of falling back
	 * to the composed configuration.
	 */
	let lastValidOverride
	...
	return {
		 ...
		 *
		 * Re-resolved on every read rather than served from a cache, because the
		 * settings layer arrives asynchronously and the interesting reads happen on
		 * both sides of it: a mount that captured the loader's configuration before
		 * the user's section was installed kept routing to the composed route for the
		 * whole process (observed live as `mounted … vision=<composed route>` while the
		 * saved section named an endpoint route).
		 *
		 * With no section, the composed configuration is in force. With a section that
		 * validates, that section wins. With a section that does not validate, the last
		 * one that did is used, so a mistyped save cannot break a working router.
		 */
		get config() {
			if (overrides === undefined || overrides === null) return resolved
			const merged = mergeOverrides(raw, overrides)
			if (merged === undefined) return resolved
			const next = normalizeConfig(merged)
			if (next.problems !== undefined) {
				trace(`settings-invalid ${next.problems.join('; ')}`)
				return lastValidOverride ?? resolved
			}
			if (!configTraced) {
				configTraced = true
				const vision = merged.vision !== null && typeof merged.vision === 'object' ? merged.vision : undefined
				trace(`config-resolved overrides=${Object.keys(overrides).join(',')} visionKeys=${vision === undefined ? 'none' : Object.keys(vision).join(',')} endpointKeys=${vision?.endpoint !== null && typeof vision?.endpoint === 'object' ? Object.keys(vision.endpoint).join(',') : 'none'} -> ${next.vision.provider}/${next.vision.model}`)
			}
			lastValidOverride = next
			return next
		},
		/** Whether a settings section is installed right now. */
		get hasOverrides() {
			return overrides !== undefined && overrides !== null
		},
		/**
		 * The raw settings section in force, key and all.
		 *
		 * The resolved configuration deliberately drops the endpoint credential, so
		 * the one consumer that must write it to the credential store reads it here
		 * rather than from a value every other reader also sees.
		 */
		get overrides() {
			return overrides
		}
...
/**
 * The configuration produced by composing the entry's own configuration with a
 * settings section.
 *
 * This is the single definition of that composition, used by `createEnv`'s live
 * getter and by anything that has to reason about what a section will resolve to.
 * Two implementations of "how a section layers over the configuration" is how a
 * base ends up stating something the composition then contradicts.
 *
 * `vision` is merged field by field and an empty value never overwrites one that
 * carries information, because the section is schema-shaped: it can name only some
 * of the vision fields the configuration already has, or none at all.
 *
 * @param raw - the entry's own configuration.
 * @param section - the settings section in force, when one is mounted.
 * @returns the composed configuration, or `undefined` when there is no section.
 */
export function composeConfig(raw, section) {
	if (section === undefined || section === null) return undefined
	const merged = mergeOverrides(raw, section)
	return merged === undefined ? undefined : normalizeConfig(merged)
}

/**
 * The composition base of the settings section: the values the card shows and the
 * host layers under the user's own section.
 * * Two properties are load-bearing, and both came out of auditing a live
 * configuration that looked ignored:
 *
 *  - **The Endpoint tier's `apiKey` is never carried.** The section is handed back
 *    by `describe()` as the resolved value, and a configuration surface seeds its
 *    editor from that; leaving the key in round-tripped it into
 *    `$DSH_HOME/settings.yaml` as plain text.
 *  - **No `vision.provider`/`vision.model` are carried.** The host composes the
 *    section DEEPLY from this base and the user's own section, so a base naming a
 *    route survives into the section beside the user's `endpoint` and reads as an
 *    explicit route — and an explicit route outranks an Endpoint, so the endpoint
 *    was ignored entirely. This is not conditional on knowing about the endpoint:
 *    the base is computed at registration time, before the user's section is read,
 *    so the configuration in hand there is the loader's and cannot know an Endpoint
 *    is about to arrive. A conditional guard therefore never fired in the live
 *    deployment (`visionKeys=provider,model,endpoint -> qwen-token-plan-cn/…`
 *    while the section's own file held only `endpoint`).
 *
 * Omitting the route costs nothing the card needs: the card displays a route by
 * looking one up, and the route in force still reaches it as the value the Host
 * resolves from `base → user section`. A configuration with no Endpoint anywhere
 * keeps the loader's route in force regardless, because that route lives in the
 * entry's own configuration, which this base layers over rather than replaces.
 *
 * @param resolved - a configuration to derive the base from.
 * @returns a schema-shaped subset, with no credential and no route.
 */
export function configBase(resolved) {
	const endpoint = resolved.endpoint === undefined
		? undefined
		: (() => {
			const { apiKey, ...rest } = resolved.endpoint
			void apiKey
			return rest
		})()
	// Only fields that cannot shadow a route: the endpoint itself (so the card can
	// edit it) and `reasoningEffort` (which the route does not contain).
	const vision = {
		...(resolved.vision.reasoningEffort === undefined ? {} : { reasoningEffort: resolved.vision.reasoningEffort }),
...
export function apply(ctx, rawConfig) {
	...
	/**
	 * The signature of the endpoint most recently written upstream, or `''` when none
	 * has been.
	 *
	 * Initialized to `''` and compared with `===`, so leaving it `undefined` made the
	 * first sync of every process look like a *withdrawal* and issue an `unset` for a
	 * route this process never wrote — deleting a route a previous session had pushed
	 * (or one the user had configured for that provider id).
	 */
	 ...
	 *    `env.config.endpoint` never carries the key at all (the resolver drops it),
	 *    so the plaintext is read here from the section that declared it — live, at
	 *    the moment of a credential write — and never travels through the
	 *    configuration value the rest of the plugin reads.
	 ...
	 */
	const declaredApiKey = () => {
		// Read from the layer in force, not from the resolution: the resolution strips
		// it deliberately.
		for (const source of [env.overrides, rawConfig]) {
			const endpoint = source !== null && typeof source === 'object' ? source.vision?.endpoint : undefined
			const key = endpoint !== null && typeof endpoint === 'object' ? endpoint.apiKey : undefined
	...
	const syncEndpointTier = (reason) => {
		syncChain = syncChain.then(async () => {
			try {
				...
				const apiKey = declaredApiKey()
				const outcome = await syncEndpoint(services, { ...configured, ...(apiKey === undefined ? {} : { apiKey }) })
...
function mount(env) {
	...
	// Which wrapper to install is decided by the mode in force at mount, and a mode
	// is a wiring change rather than a per-call decision. The ROUTE is not: both
	// wrappers read `env.config` per call, so a section saved after the mount takes
	...
	const live = env.config
	trace(`mounted mode=${live.mode} vision=${live.vision.provider}/${live.vision.model} settings=${env.hasOverrides ? 'yes' : 'no'} maxTokens=${live.maxTokens} timeoutMs=${live.timeoutMs} label=${live.label} dryRun=${live.dryRun}`)
	if (live.mode === 'digest') {
		ctx.logger?.info?.(`image-router: mounted in digest mode — images are analysed by ${live.vision.provider}/${live.vision.model} and replaced with text; the session model is never changed`)
		return
	}
	ctx.logger?.info?.(`image-router: mounted in switch mode — image turns → ${live.vision.provider}/${live.vision.model}${live.sticky ? ' (sticky: the lent route is never released)' : `, released after ${live.holdTurns} image-free turn(s)`}${live.dryRun ? ' [dryRun]' : ''}`)
}
