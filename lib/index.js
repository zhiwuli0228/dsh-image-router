...
async function analyzeImages(env, refs, sessionId, signal) {
	...
	let failure
	let finishReason
	/** The distinct chunk `type`s this route emitted, in order (bounded). */
	const shapes = []
	try {
		for await (const chunk of llm.stream(options)) {
			...
			if (chunk.type === 'text-delta' && typeof chunk.text === 'string') {
				...
				// The adapter emits the completed block here as well as any deltas
				// (`text_end` → `block-end` with `block: { type: 'text', text }`), and a
				// provider that does not stream deltas delivers the whole answer ONLY
				// this way. Reading just `text-delta` therefore produced an empty
				// analysis that looked like the model answering nothing, with a `stop`
				// finish and no error anywhere.
				if (typeof chunk.block.text === 'string' && chunk.block.text.length > text.length) text = chunk.block.text
			} else if (chunk.type === 'finish') {
				finishReason = chunk.reason ?? chunk.finishReason ?? finishReason
				if (chunk.reason === 'error' || chunk.reason === 'aborted') failure = chunk.reason
				// A provider may report the failure on the finish chunk instead of throwing.
				else if (chunk.error !== undefined) failure = chunk.error?.message ?? String(chunk.error)
	...
	const observed = `chunks=${shapes.length === 0 ? 'none' : shapes.join('+')}${finishReason === undefined ? '' : ` finish=${finishReason}`}`
	const trimmed = text.trim()
	if (failure !== undefined) {
		env.trace(`digest-failed reason=${failure} images=${refs.length} ${observed}`)
		return undefined
	}
	if (trimmed.length === 0) {
		// An empty answer is the one failure a user cannot diagnose from the outside:
		// the route resolved, the credential resolved, and nothing came back. The chunk
		// shapes and the finish reason say whether the adapter produced a different
		// shape than expected, stopped on a length budget, or answered nothing at all.
		env.trace(`digest-empty images=${refs.length} ${observed} — the route produced no text`)
		return undefined
	}
	return trimmed
}
