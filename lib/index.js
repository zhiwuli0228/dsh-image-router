...
async function loadConfigSchema() {
	...
	if (z === undefined || typeof z.object !== 'function') {
		 ...
		 *
		 * Calling it the way schemastery is called (`schema()` to materialize
		 * defaults, `schema(value)` to coerce one) answers with the documented
		 * defaults merged with whatever was supplied. It carries no validation
		 * because the semantic checks live in {@link normalizeConfig}; a schema
		 * that rejected a half-filled deployment would fail the boot instead of
