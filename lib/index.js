 ...
 *
 * The Endpoint tier's `apiKey` is **dropped here**, and that is load-bearing. The
 * settings section is handed back by `describe()` as the resolved value — which
 * includes the user layer — and a configuration surface seeds its editor from
 * that. Leaving the key in meant a save (or even an unrelated edit) round-tripped
 * it into `$DSH_HOME/settings.yaml` as plain text: a secret in a configuration
 * file, which is exactly what this plugin promises never happens. The key is
 * written once to the credential store and is write-only from then on.
 *
...
export function configBase(resolved) {
	...
	return {
		...
		vision: endpoint === undefined ? resolved.vision : { ...resolved.vision, endpoint },
