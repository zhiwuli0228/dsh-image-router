 ...
 *
 * Even at that path the write **replaces** the profile object rather than
 * merging into it (`applyPathOp`: `{...section, [head]: op.value}`), so whatever
 * the Models page added to this route — `headers`, `compat`, a longer
 * `timeoutMs` — would be dropped by a save from this card. The route this card
 * writes is also a route the user can edit there, so the existing profile is
 * merged under the fields this card owns.
 *
 * @param endpoint - a normalized endpoint.
 * @param readProfile - reads the current `providers[ENDPOINT_PROVIDER]` value.
 * @returns one settings path operation.
 */
export function endpointSettingsOp(endpoint, readProfile) {
	const mine = endpointProfile(endpoint)
	let existing
	try {
		existing = typeof readProfile === 'function' ? readProfile() : undefined
	} catch {
		existing = undefined
	}
	const value = existing !== null && typeof existing === 'object' && !Array.isArray(existing)
		? { ...existing, ...mine, models: mine.models }
		: mine
	return { op: 'set', path: ['providers', ENDPOINT_PROVIDER], value }
}
 ...
 *
 * @param services - `{ settings, readProfile, trace }`; any may be absent.
...
export async function syncEndpoint(services, endpoint) {
	...
	try {
		await settings.mutate('llm-pi-ai', [endpointSettingsOp(endpoint, services.readProfile)])
	} catch (error) {
